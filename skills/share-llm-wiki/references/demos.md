...
六个真实场景 + 一组反面样例，输出为 2026-09-18 实跑快照；命令已随脚本更新（资产筛选使用完整输出）。
...
那两个 `sources:` 指向的是 **YAML 与 Shell 资产，不是 Markdown**。它们**可以直接读**（2026-09-18 起）：

```bash
./scripts/wiki.sh assets --all sources/experiment-runs | grep v0017
...
**这一条救了什么**：这对应方法论文的游戏 wiki 案例——约 **48–70%** 被报为
「wiki 缺失」的内容其实已经存在，只是检索没命中；该比例不是本 VLA 库的测量结果。第一次搜空，
...
| 断言「库里没有」而只搜过一次 | 方法论文案例出现大量检索假阴性（样例 4） |
