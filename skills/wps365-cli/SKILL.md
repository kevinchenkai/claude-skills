...
- 一次最多约 20 个 id。
- **搬家优先用上面的精装命令 `drive file batch-move`，别自己去打 `api post`。**
  实测绕路的代价：手写请求体时把字段猜成 `target_parent_id`，真实字段是
  `dst_parent_id` + `dst_drive_id`（两个都必填）。精装命令的 flag 名与字段一一对应，
  且同样支持 `--dry-run`，没有任何理由手写。
...
🔴 **`>20MB 进附件/` 只管二进制附件，不管 `.otl`。** 智能文档天然会很大（内嵌图片），
本仓一次实测 39 个 `.otl` 里有 6 个超过 20MB，全是正文主文档。
把它们挪进 `附件/` 是把正文当附件，属于治理事故。判据看**扩展名**，不是只看体积。

🔴 **名字带 `-copy` / `副本` 的，删之前必须比内容。** 一次治理里实测遇到两种，都不能删：
- 一份 `X-copy.otl` 比原件 `X.otl` **多 9 个 block、多 477 字**（正文相似度 0.93）——
  是后来在副本上继续编辑的，删了就丢内容；
- 另一份 `Y-copy.otl` **全目录根本没有对应的 `Y.otl`**——它自己就是正本，
  只是名字带了个误导后缀。

  比法：体积相同才有嫌疑，但体积不同**不代表**无关；要按 §6 读 blocks 比正文字数与
  block 数。`-copy` 是命名习惯，不是"可删"的标记。

**目录已经治理过的情况**（已有 `01.`…`99.` 编号）不要推倒重来，只补缺口：
根目录散落的归位、大附件归位。发现某个目录名与要归入的文件语义不符时，
**改目录名比把文件塞进错目录好**（`drive file rename <drive> <id> --name "新名"`，
实测可用于文件夹）。

流程：递归清单 → 给归位表 → **等确认** → create + move → 删空旧夹 → 回传最终树。
（用户明说"不用确认"时跳过确认那步，**其余步骤一步不能省**——尤其是先存清单和事后集合比对。）

🔴 **验收必须是集合比对，不是"看着对"**：治理前后各拉一次递归清单，比对
**文件 id 集合**和**总字节数**。本仓一次实测 43 个文件、414,679,290 字节前后完全一致。
只数条数会漏掉"移动时被覆盖/改名"的情况。
